...
func (db *DB) GetJobData(ctx context.Context, jobId int) (types.JobData, error) {
	...
	query := fmt.Sprintf(`
			SELECT j.name, j.streams_config, j.state, j.project_id, s.config, d.config, s.version, s.type, COALESCE(j.advanced_settings::text, '')
			FROM %q j
	...
	if err := rows.Scan(&jobData.JobName, &jobData.Streams, &jobData.State, &jobData.ProjectID, &jobData.Source, &jobData.Destination, &jobData.Version, &jobData.Driver, &jobData.AdvancedSettings); err != nil {
