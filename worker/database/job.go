...
func (db *DB) GetJobData(ctx context.Context, jobId int) (types.JobData, error) {
	...
	if err := rows.Scan(&jobData.JobName, &jobData.Streams, &jobData.State, &jobData.ProjectID, &jobData.Source, &jobData.Destination, &jobData.Version, &jobData.Driver, &jobData.AdvancedSettings,
